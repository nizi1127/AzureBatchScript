...
class Configuration:
    ...
    def __init__(self, file_name):
        ...
        try:
            ...
            self.workspace_file_share = conf['workspace_settings'].get('file_share_name', 'fs')
            self.workspace_resource_group = conf['workspace_settings'].get('resource_group_name', 'myrg')
            self.workspace = conf['workspace_settings'].get('workspace_name', 'myws')
            self.workspace_cluster = conf['workspace_settings'].get('cluster_name', 'cluster')
            self.workspace_relative_mount_path = conf['workspace_settings'].get('relative_mount_path', 'mount')
            self.workspace_vm_size = conf['workspace_settings'].get('vm_size', 'STANDARD_NC6')
            self.workspace_node_count = conf['workspace_settings'].get('node_count', 1)
